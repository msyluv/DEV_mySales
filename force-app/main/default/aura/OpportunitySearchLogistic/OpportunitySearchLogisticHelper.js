({
 ...
 //TODO end
				...
				component.set('v.customerType', customerType);
				component.set('v.selectedValue.customerType', customerType[0].value);
				
				component.set('v.bizDevelopment', bizDevelopment);
				component.set('v.selectedValue.bizDevelopment', bizDevelopment[0].value);
				component.set('v.lbsExceptionType', lbsExceptionType);
				component.set('v.selectedValue.lbsExceptionType', lbsExceptionType[0].value);
	...
	getOppList : function(component, event,rcdId, searchType) {
      ...
      //  var CSPCheck = component.get('v.CSPCheck'); //Added by Anish - v 1.2
        var globalBiddingCheck = component.get('v.globalBiddingCheck');
     ...
     //   var MESCheck = component.get('v.MESCheck'); //Added by Anish - v 1.2
		...
		if(selectedValue.StartDate != null && selectedValue.EndDate == null){
			self.showMyToast('error', $A.get( "$Label.c.BOSEARCH_CloseDate_MSG_0001")); // Please select CloseDate(End)
			component.set('v.loading', false);
			return;
		}
		
		if(selectedValue.EndDate != null && selectedValue.StartDate == null){
			self.showMyToast('error', $A.get( "$Label.c.BOSEARCH_CloseDate_MSG_0002")); // Please select CloseDate(Start)
			component.set('v.loading', false);
			return;
		}
      
		if(selectedValue.StartDate > selectedValue.EndDate ){
			self.showMyToast('error', $A.get( "$Label.c.BOSEARCH_CloseDate_MSG_0003"));
			component.set('v.loading', false);
			return;
		} 
        
        //Start v 1.3
        if(selectedValue.ContractStartDate != null && selectedValue.ContractEndDate == null){
			self.showMyToast('error', $A.get( "$Label.c.BOSEARCH_ContractDate_MSG_0001"));
			component.set('v.loading', false);
			return;
		}
		
		if(selectedValue.ContractEndDate != null && selectedValue.ContractStartDate == null){
			self.showMyToast('error', $A.get( "$Label.c.BOSEARCH_ContractDate_MSG_0002"));
			component.set('v.loading', false);
			return;
		}

		if(selectedValue.ContractStartDate >  selectedValue.ContractEndDate){
			self.showMyToast('error', $A.get( "$Label.c.BOSEARCH_ContractDate_MSG_0003"));
			component.set('v.loading', false);
			return;
		} 
     ...
     //   bTechAttribute : bTechAttribute,  //Anish by Anish - v 1.1  /! to 
     //       CSPCheck : CSPCheck, //Added by Anish - v 1.2
	        globalBiddingCheck : globalBiddingCheck, 
