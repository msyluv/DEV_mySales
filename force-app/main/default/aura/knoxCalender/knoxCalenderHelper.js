...
({
	doInit : function(component, event){
		...
		//인프라 작업 등 공지용 메세지
		if('Y'==$A.get('$Label.c.MAIN_NOTICE_FLAG')){
			self.showToast('other', $A.get('$Label.c.MAIN_NOTICE_TITLE'), $A.get('$Label.c.MAIN_NOTICE_MSG'));
		}
		...
		self.queryActivity(component, event);
        	...
        	self.apex(component, 'getcalendarlist', { })
		.then(function(result){
			window.console.log('result : ', result);
		})
		.catch(function(errors){
	...
	queryActivity : function(component, event){
		...
		.then(function(result){
            ...
            console.log('Atul123==>'+JSON.stringify(component.get('v.activity')));
	...
	drawCalendar : function(component, event) {
        ...
        window.setTimeout( ()=> {
			var calFrame = component.find("uniqueDiveId").getElement();
			...
			calFrame.addEventListener('click', function(ev){
                console.log('divid___________' + component.find("uniqueDiveId"));
				...
				console.log('targetId________'+ targetId);
				// 클릭된 div 배경색 변경
				var targetObj2  = document.getElementById(targetId);
                console.log('targetId________'+ targetId);
                console.log('targetObj_____' + targetObj2);
				var beforetargetObj  = document.getElementsByClassName('setSelected')[0];
				$A.util.removeClass(beforetargetObj,'setSelected');
				if( targetObj2 != null) $A.util.addClass(targetObj2,'setSelected');
			...
			for(var j = 0; j < 7; j++) {
				...
				if(nDay != ""){
					if(activity.event[date] != undefined){
                        ...
                        console.log('EventInfo==>'+JSON.stringify(activity));*/
                        let Val = activity.event[date];
                        var Val1 = Val.toString().split('-');
                        console.log('Val1'+Val1);
                        var Val2 =  Val1[0];
                        var SubName = Val1[1];
                        var StartTime = Val1[2];
                        var EndTime = Val1[3]; 
                        console.log('Val2'+Val2);
						let eventArr = activity.event[date];
                        console.log('Testing2'+eventArr);
						//let evtIdx 	 = eventArr.indexOf( "Event"); Comment by Atul
						let evtIdx 	 = Val2.indexOf( "Event");
                        console.log('Testing3'+evtIdx);
                        console.log('EventInfo==>'+JSON.stringify(activity));
						if( evtIdx  > -1){
							calBody += "<span class='event' style='font-weight:bold'>"+SubName+"</br>&nbsp;&nbsp;"+StartTime+'-'+EndTime+"</span>";
    ...
    callKnoxData : function(component , event){
        var self = this;
        var bjobid;
        var jobstatus;
        self.apex(component, 'getKnoxSchedules', { })
		.then(function(result){
			window.console.log('result123 : ', result);
           if (result != ''){
               bjobid = result
               self.apex(component, 'getAsnycjobStatus', {jobid :bjobid })
		.then(function(result){
           ...
           while(jobstatus != 'Completed'){
                 jobstatus = self.getasyncjobstatus(component,bjobid);
 
                console.log('divyam12356',jobstatus);
               if(component.get("v.batchstatus") == 'Completed')
                   break;
    ...
    getasyncjobstatus : function(component,bjobid){
        var self = this;
        var statusval;
        console.log('cometolast');
         self.apex(component, 'getAsnycjobStatus', {jobid :bjobid })
		.then(function(result){
			window.console.log('result23 : ', result);
             statusval = result;
            component.set("v.batchstatus",statusval);
		})
		.catch(function(errors){
			self.errorHandler(errors);
		});
